...
: tracker_state(LOST),
  ...
  measurement(Eigen::Matrix<double, MEASUREMENT_DIM ,1>::Zero()),
  target_state(Eigen::Matrix<double, STATE_DIM, 1>::Zero()),
...
{
  // KF predict
  Eigen::Matrix<double, STATE_DIM, 1> ekf_prediction = ekf.predict();
    ...
    for (const auto & armor : armors_msg->armors) {
      ...
      if (armor.number == tracked_id) {
        ...
        if (position_diff < min_position_diff) {
          ...
          // 原本的代码是：ekf_prediction(6)，但是第6个元素是z轴坐标，改为第9个元素
  ...
  if (target_state(12) < 0.12) {
    target_state(12) = 0.12;
    ekf.setState(target_state);
  } else if (target_state(12) > 0.4) {
    target_state(12) = 0.4;
    ekf.setState(target_state);
  }
...
void Tracker::handleArmorJump(const Armor & current_armor)
{
  ...
  if (tracked_armors_num == ArmorsNum::NORMAL_4) {
    ...
    std::swap(target_state(12), another_r);
  }
  ...
  if ((current_p - infer_p).norm() > max_match_distance_) {
    ...
    target_state(10) = 0;                  // v_yaw
    target_state(11) = 0;                  // a_yaw
...
Eigen::Vector3d Tracker::getArmorPositionFromState(const Eigen::Matrix<double,STATE_DIM, 1> & x)
{
