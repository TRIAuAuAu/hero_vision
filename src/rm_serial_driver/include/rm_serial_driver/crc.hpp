...
inline void Append_CRC16_Check_Sum(uint8_t * pchMessage, uint32_t dwLength)
{
  ...
  while (dwLength--) {
    chData = *pchMessage++;
    wCRC = (uint16_t)((wCRC >> 8) ^ CRC16_TABLE[(wCRC ^ chData) & 0xff]);
  }
  
  *pchMessage++ = (uint8_t)(wCRC & 0xff);
  *pchMessage = (uint8_t)((wCRC >> 8) & 0xff);
}

inline uint32_t Verify_CRC16_Check_Sum(const uint8_t * pchMessage, uint32_t dwLength)
{
  uint16_t wCRC = CRC_INIT;
  uint8_t chData;
